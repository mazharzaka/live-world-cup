async function verify() {
  console.log("Fetching new schedule from http://localhost:3001/api/schedule...");
  try {
    ...
    matches.forEach(m => {
      ...
      allLinks.forEach(link => {
        const isBad = link.includes("poiy.online") || 
                      link.includes("fila7lam.com") || 
                      link.includes("live-soccer.tv") ||
                      link.includes("blogspot.com") ||
                      /\/\d{4}\/\d{2}\//.test(link) ||
                      ["scholarship", "insurance", "loan", "investing", "wealth", "estate"].some(kw => link.includes(kw));
